...
async function scrape(url) {
  ...
  try {
    ...
    // ==========================================
    // THE CSS UNHIDE INJECTION
    // ==========================================
    // Force all hidden tabs and accordions to become visible 
    // so innerText can actually read them.
    await page.evaluate(() => {
      const allElements = document.querySelectorAll('*');
      allElements.forEach(el => {
        const style = window.getComputedStyle(el);
        if (style.display === 'none') {
          // Force display block with !important to override inline styles
